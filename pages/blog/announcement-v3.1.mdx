...
One of the biggest changes is that we use our **own framework** for connecting to telegram instead of using gramjs like in v2.

Apart from that, we also make all JSON follow **bot api style**. Because we want people to not have to adapt when moving from api bot to MTProto
